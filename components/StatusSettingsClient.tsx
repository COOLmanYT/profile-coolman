...
export default function StatusSettingsClient({ initialSettings }: { initialSettings: StatusSettings }) {
  ...
  const save = async () => { setSaving(true); setMessage(null); try { const response = await fetch('/api/dashboard/status', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(settings) }); if (!response.ok) throw new Error(await responseErrorCode(response)); setMessage('Saved'); showToast({ variant: 'success', title: 'Instatus mapping saved' }) } catch (error) { setMessage('Could not save settings'); showToast({ variant: 'error', title: 'Could not save Instatus mapping', code: error instanceof Error ? error.message : 'NETWORK_ERROR' }) } finally { setSaving(false) } }
